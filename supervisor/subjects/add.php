...
?>
	<div class="section" id="login" style="margin-bottom: 33px;">
      <div class="container">
        <h2 class="special-heading">Add Subject</h2>
        <form action="<?php echo $cont."SubjectController.php?method=add"?>" method="POST" class="form">
          <?php
            if(isset($_SESSION['errors'])) {
          ...
          <div style="display: flex;justify-content:center;flex-direction:column">
            ...
            <div>
                ...
                value="<?php if(isset($_SESSION['errors'])) { echo $code ;} ?>">
            </div>
            <div>
                ...
                value="<?php if(isset($_SESSION['errors'])) { echo $section_number ;} ?>">
            </div>
            <div>
                <label for="type">Type</label>
                <select name="type" id="type" title="Choose Type" aria-placeholder="Choose Type">
                    <option value="ولاد" <?php if(isset($_SESSION['errors']) && $type == 'ولاد') echo 'selected'?>>ولاد</option>
                    <option value="بنات" <?php if(isset($_SESSION['errors']) && $type == 'بنات') echo 'selected'?>>بنات</option>
                </select>
            </div>
            <div>
                ...
                <select name="course_id" id="course_id" title="Choose Course" aria-placeholder="Choose Course">
                    <?php foreach($courses as $course) { ?> 
                        <option value="<?php echo $course['id'] ?>" <?php if(isset($_SESSION['errors']) && $course_id == $course['id']) echo 'selected'?>><?php echo $course['name']?></option>
                    <?php } ?>
                </select>
            </div>
            <div>
                ...
                <select name="professor_id" id="professor_id" title="Choose professor" aria-placeholder="Choose professor">
                    <?php foreach($professors as $professor) { ?> 
                        <option value="<?php echo $professor['id'] ?>" <?php if(isset($_SESSION['errors']) && $professor_id == $professor['id']) echo 'selected'?>><?php echo $professor['name']?></option>
                    <?php } ?>
                </select>
            </div>
            <div>
                ...
                <select name="semester_id" id="semester_id" title="Choose Semester" aria-placeholder="Choose Semester">
                    <?php foreach($semesters as $semester) { ?> 
                        <option value="<?php echo $semester['id'] ?>" <?php if(isset($_SESSION['errors']) && $semester_id == $semester['id']) echo 'selected'?>><?php echo $semester['name']." ".$semester['year']?></option>
                    <?php } ?>
                </select>
            </div>
            <div>
                ...
                <select name="class_id" id="class_id" title="Choose Class" aria-placeholder="Choose Class">
                    <?php foreach($classes as $class) { ?> 
                        <option value="<?php echo $class['id'] ?>" <?php if(isset($_SESSION['errors']) && $class_id == $class['id']) echo 'selected'?>><?php echo $class['building_number']."/".$class['name']?></option>
                    <?php } ?>
                </select>
            </div>
            <div>
                ...
                <select name="days[]" id="days[]" title="Choose Days" aria-placeholder="Choose Days" multiple required>
                    <?php foreach($_SESSION['days'] as $day) { ?> 
                        <option value="<?php echo $day['id'] ?>" <?php if(isset($_SESSION['errors']) && in_array($day['id'],$days)) echo 'selected'?>><?php echo $day['name']?></option>
                    <?php } ?>
                </select>
            </div>
            <div>
