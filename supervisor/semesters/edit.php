...
?>
	<div class="section" id="login" style="margin-bottom: 33px;">
      <div class="container">
        <h2 class="special-heading">Edit Semester</h2>
        <form action="<?php echo $cont."SemesterController.php?method=edit"?>" method="POST" class="form">
          <?php
            if(isset($_SESSION['errors'])) {
          ...
          <div style="display: flex;justify-content:center;flex-direction:column">
            ...
            <div>
                ...
                value="<?php if(isset($_SESSION['errors'])) { echo $name ;} else { echo $semester['name'];} ?>">
            </div>
            <div>
                <label for="year">Year</label>
                <input type="text" name="year" id="year" title="Enter Year" placeholder="Enter Year" required
                value="<?php if(isset($_SESSION['errors'])) { echo $year ;} else { echo $semester['year'];} ?>">
            </div>
            <div>
                <input type="submit" name="edit_semester" value="Save Changes">
            </div>
          </div>
        </form>
      </div>
